...
Reading an exchange (text, email or meeting), write the summary so it says who asked, offered or promised each thing: Ethan or the Person. Record a Fact from it only when the Person plainly says it about themselves, such as their employer, city, school or role: pass those as `facts` (short lowercase key, value) on the same `add_interaction`, up to three. What Ethan says about them, what they say about a third party, plans, guesses and anything in `avoid_topics` are not Facts. Each Fact is saved pointing at its Interaction. The result marks each `set`, `unchanged` or `skipped`; a `skipped` Fact means one already stands (Ethan's own, or from a newer message), so leave it.
...
2. Scan from senders and recipients alone. For email, search Gmail with `gmail_query` from parameters.md plus `after:` `since`. For iMessage, run `sh .claude/skills/people/imessage-scan.sh <since>`: the iMessage connector cannot list conversations, so the script counts from the local Messages database and never selects message text. It prints one line per one-to-one conversation: identifier, `sent`, `received`, last message time, newest message GUID (group chats and tapbacks are left out). Identifiers that are not a phone number or email, and short codes of 5 or 6 digits, are `automated`. If the script fails (for example the database cannot be opened because Full Disk Access is missing), the source failed: do not fall back to `get_unread_imessages`, which returns text. Group by identifier and count messages Ethan sent (`sent`) and received (`received`). Set `automated` for list, bulk or system senders. Message text stays unread at this stage.
3. For iMessage, name the phone-number candidates that clear the thresholds with `sh .claude/skills/people/contact-names.sh <phone> ...`, which reads the local Contacts database (the connector's `search_contacts` matches names only and needs the Contacts app open). Use the handle as the name for anyone it does not return. Call `suggest_people` with every candidate and the three `suggest_*` and `ignore_*` values from parameters.md. Results come back in candidate order.
4. Read message text only for `has_file` people; skipped people are never opened. For iMessage, read a has_file person's texts with `read_imessages` on their phone number. For each has_file person, write one Interaction per email thread or per day of texts with `add_interaction`: source `email` or `imessage`, `direction` `in` when they wrote last, `source_ref` the id of the newest message the summary covers (Gmail message id, iMessage message GUID), and `refresh_profile` false on every entry except that person's last. Follow the summary and Fact rules under Adding a note.
