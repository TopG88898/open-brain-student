...
function profilePrompt(
  ...
  return [
    ...
    `The limit is a hard cap: never go over ${maxWords} words. Aim for about ${Math.round(maxWords * 0.75)} words, and drop the least important details rather than run long.`,
    'State only what the facts and interactions say. Do not speculate, interpret, or comment on their significance. Do not hedge with words like "appears", "apparently" or "seemingly": if an entry does not say it, leave it out. Do not infer a plan, intention or offer that an entry does not state. Do not quote messages verbatim.',
